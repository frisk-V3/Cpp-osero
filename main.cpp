...
bool IsValid(int r, int c, Space s) {
    if (r < 0 || r >= SIZE || c < 0 || c >= SIZE || board[r][c] != Space::EMPTY) return false;
    for (int dr = -1; dr <= 1; ++dr) {
        for (int dc = -1; dc <= 1; ++dc) {
            if (dr == 0 && dc == 0) continue;
            int nr = r + dr, nc = c + dc;
            bool found_opp = false;
            while (nr >= 0 && nr < SIZE && nc >= 0 && nc < SIZE && board[nr][nc] == (s == Space::BLACK ? Space::WHITE : Space::BLACK)) {
...
void Flip(int r, int c, Space s) {
    ...
    for (int dr = -1; dr <= 1; ++dr) {
        for (int dc = -1; dc <= 1; ++dc) {
            ...
            if (!targets.empty() && nr >= 0 && nr < SIZE && nc >= 0 && nc < SIZE && board[nr][nc] == s) {
...
void DrawBoard(HDC hdc) {
    ...
    Rectangle(hdc, BOARD_MARGIN, BOARD_MARGIN, BOARD_MARGIN + SIZE * CELL_SIZE, BOARD_MARGIN + SIZE * CELL_SIZE);
    DeleteObject(hBrush);

    for (int i = 0; i <= SIZE; ++i) {
        MoveToEx(hdc, BOARD_MARGIN + i * CELL_SIZE, BOARD_MARGIN, NULL);
        LineTo(hdc, BOARD_MARGIN + i * CELL_SIZE, BOARD_MARGIN + SIZE * CELL_SIZE);
        MoveToEx(hdc, BOARD_MARGIN, BOARD_MARGIN + i * CELL_SIZE, NULL);
        LineTo(hdc, BOARD_MARGIN + SIZE * CELL_SIZE, BOARD_MARGIN + i * CELL_SIZE);
    }
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
    const wchar_t CLASS_NAME[] = L"OthelloWindowClass";
    WNDCLASS wc = {};
